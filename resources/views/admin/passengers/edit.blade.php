    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.passengers.update", [$passenger->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $passenger->nombre) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('dni') ? 'is-invalid' : '' }}" type="text" name="dni" id="dni" value="{{ old('dni', $passenger->dni) }}">
                @if($errors->has('dni'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('vuelos') ? 'is-invalid' : '' }}" name="vuelos[]" id="vuelos" multiple>
                    @foreach($vuelos as $id => $vuelo)
                        <option value="{{ $id }}" {{ (in_array($id, old('vuelos', [])) || $passenger->vuelos->contains($id)) ? 'selected' : '' }}>{{ $vuelo }}</option>
                    @endforeach
                </select>
