...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.flight.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.flights.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ruta') ? 'is-invalid' : '' }}" type="text" name="ruta" id="ruta" value="{{ old('ruta', '') }}">
                @if($errors->has('ruta'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pasajeros') ? 'is-invalid' : '' }}" name="pasajeros[]" id="pasajeros" multiple>
                    @foreach($pasajeros as $id => $pasajero)
                        <option value="{{ $id }}" {{ in_array($id, old('pasajeros', [])) ? 'selected' : '' }}>{{ $pasajero }}</option>
                    @endforeach
                </select>
